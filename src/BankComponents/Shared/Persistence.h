...
private:
	...
	const string CREATE_CUSTOMER_TO_ACCOUNT = "CREATE TABLE IF NOT EXISTS CUSTOMER_TO_ACCOUNT(CUSTOMER_ID INTEGER, ACCOUNT_ID INTEGER, PRIMARY KEY(CUSTOMER_ID, ACCOUNT_ID), FOREIGN KEY(CUSTOMER_ID) REFERENCES CUSTOMER(ID), FOREIGN KEY(ACCOUNT_ID) REFERENCES ACCOUNT(ACCOUNT_NUMBER));";
